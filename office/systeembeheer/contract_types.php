...
/* ======================================================
   ACTIES (toevoegen / bijwerken / status wisselen)
====================================================== */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['actie']) && $_POST['actie'] === 'toevoegen') {
        ...
        $stmt = $conn->prepare("INSERT INTO contract_types (naam, beschrijving, actief) VALUES (?, ?, 1)");
        $stmt->bind_param("ss", $naam, $beschrijving);
        $stmt->execute();
        $stmt->close();

        setFlash("Contracttype toegevoegd.", "success");
        header("Location: contract_types.php");
        exit;
    }
    ...
    if (isset($_POST['actie']) && $_POST['actie'] === 'bewerken') {
        ...
        setFlash("Contracttype bijgewerkt.", "success");
        header("Location: contract_types.php");
        exit;
    }

    // Actief / Inactief togglen
    if (isset($_POST['actie']) && $_POST['actie'] === 'toggle') {
        $id = intval($_POST['type_id']);

        $conn->query("UPDATE contract_types SET actief = 1 - actief WHERE type_id = $id");
...
$pageTitle = "Contracttypen beheren";
ob_start();
?>

<div class="page-header" style="display:flex;justify-content:space-between;align-items:center;">
    <h2>📄 Contracttypen</h2>
    <button class="btn btn-accent" onclick="openNieuwType()">➕ Nieuw type</button>
</div>
...
<div class="card">
    <table class="data-table">
        ...
        <?php if ($result->num_rows == 0): ?>
            <tr><td colspan="4" style="text-align:center;color:#777;">Geen contracttypen gevonden.</td></tr>
        <?php endif; ?>

        <?php while ($ct = $result->fetch_assoc()):
            $statusLabel = $ct['actief'] ? "🟢 Actief" : "🔴 Inactief";
            $statusColor = $ct['actief'] ? "#2e7d32" : "#c62828";
        ?>
            <tr>
                <td><?= htmlspecialchars($ct['naam']) ?></td>
                <td><?= htmlspecialchars($ct['beschrijving']) ?></td>
                <td><span style="color:<?= $statusColor ?>; font-weight:600;"><?= $statusLabel ?></span></td>
                <td class="actions">

                    <!-- Bewerken -->
                    <a href="#" onclick='openBewerkType(<?= json_encode($ct) ?>)'>✏️</a>

                    <!-- Actief/Inactief -->
                    <form method="post" style="display:inline;">
                        <input type="hidden" name="actie" value="toggle">
                        <input type="hidden" name="type_id" value="<?= $ct['type_id'] ?>">
                        <button class="link-btn" onclick="return confirm('Status wijzigen?');">
                            🔄
                        </button>
                    </form>
    ...
    <div class="popup-content">
        ...
        <form method="post">
            ...
            <div class="popup-buttons">
                ...
                <button class="btn btn-secondary" type="button" onclick="closePopup('popupNieuw')">Annuleren</button>
            </div>
        </form>
    </div>
</div>
...
<div class="popup-overlay" id="popupBewerk" style="display:none;">
    <div class="popup-content">
        <h3>✏️ Contracttype bewerken</h3>

        <form method="post" id="bewerkForm">
            ...
            <label>Beschrijving</label>
            <textarea name="beschrijving" id="edit_beschrijving" rows="3"></textarea>
